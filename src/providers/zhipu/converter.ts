 ...
 * at https://open.bigmodel.cn/api/paas/v4/chat/completions).
 *
 * Codex sends:
 *   { model, instructions, input: string | InputItem[], tools?, stream, store, ... }
 *
 * GLM accepts the Chat Completions shape:
...
function convertInputItem(item: unknown, out: ChatMessage[], state: ConvertState): void {
  ...
  if (type === 'reasoning') {
    ...
    // (GLM thinking mode requires reasoning_content to be passed back).
  ...
  if (type === 'function_call') {
    ...
    // tool messages can follow as a block (GLM rejects an assistant tool_calls
