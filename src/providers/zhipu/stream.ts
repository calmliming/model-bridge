...
/**
 * Converts an OpenAI Chat Completions SSE stream (what Zhipu GLM sends back
 * from /api/paas/v4/chat/completions) into an OpenAI Responses-API SSE event
...
// field verbatim. We read it back in converter.ts so GLM's reasoner gets
// `reasoning_content` on every echoed assistant turn (otherwise GLM may reject
