 ...
 * an OpenAI Chat Completions request body (what Kimi's Moonshot upstream accepts
 * at https://api.moonshot.cn/v1/chat/completions).
 *
 * Codex sends:
 *   { model, instructions, input: string | InputItem[], tools?, stream, store, ... }
 *
 * Moonshot accepts the Chat Completions shape:
...
/**
 * Codex defaults to model names like `gpt-5.5` which Moonshot does not
 * recognise. Rewrite anything that doesn't already look like a Kimi/Moonshot
 * model to `kimi-k3` (the flagship, and the only default that is valid on BOTH
 * the chat/completions endpoint and the Anthropic-compatible endpoint — the
 * `kimi-k3[1m]` form Claude Code documents is a 1M-context annotation that the
 * chat endpoint rejects). Anything starting with `kimi` or `moonshot` is passed
 * through, so users can set `model="kimi-k3[1m]"` for the 1M window on the
 * Anthropic endpoint, `model="kimi-k2.7-code"` for the coding model, etc.
 */
...
export function responsesToChatCompletions(
  ...
  // Kimi thinking-mode "all-or-nothing" rule: if ANY assistant message in the
  // history carries `reasoning_content`, every other assistant message —
  // including tool-call-only ones — must carry it too, or the API may reject
  // the whole request with `reasoning_content ... must be passed back`. Codex
  // CLI sometimes drops the reasoning text on echo (older sessions, or when the
...
function convertInputItem(item: unknown, out: ChatMessage[], state: ConvertState): void {
  ...
  if (type === 'reasoning') {
    ...
    // (Kimi thinking mode requires reasoning_content to be passed back).
  ...
  if (type === 'function_call') {
    ...
    // tool messages can follow as a block (Kimi rejects an assistant tool_calls
    // message that isn't immediately followed by its tool replies).
  ...
  if (typeof content === 'string') {
    ...
    for (const part of content) {
      ...
      // input_image / image_url / file / etc. silently skipped — the Kimi
