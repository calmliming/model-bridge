...
/**
 * Usage parser for the Kimi → Responses adapter. It consumes events AFTER the
 * StreamTransform in `stream.ts` has rewritten Chat Completions chunks into
 * Responses-API events, so the relevant payload is the same shape as the OpenAI
 * Responses upstream: `response.completed` carries the final `response.usage`.
 */
 ...
 * `forceStream: true` so /api/kimi/v1/responses always streams. Kept
