...
/**
 * Converts an OpenAI Chat Completions SSE stream (what Kimi's Moonshot gateway
 * sends back from /v1/chat/completions) into an OpenAI Responses-API SSE event
 * stream (what Codex CLI expects on /v1/responses).
 *
...
interface ChatUsage {
  ...
  // Moonshot reports context-cache hits under `cached_tokens`.
...
// field verbatim. We read it back in converter.ts so Kimi's reasoner gets
// `reasoning_content` on every echoed assistant turn (otherwise Kimi may reject
// the call with `reasoning_content must be passed back to the API`).
...
export function createKimiResponsesStreamTransform(): StreamTransform {
