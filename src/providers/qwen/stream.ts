...
/**
 * Converts an OpenAI Chat Completions SSE stream (what Qwen's DashScope gateway
 * sends back from /compatible-mode/v1/chat/completions) into an OpenAI
 * Responses-API SSE event stream (what Codex CLI expects on /v1/responses).
 *
...
// field verbatim. We read it back in converter.ts so Qwen's reasoner gets
// `reasoning_content` on every echoed assistant turn (otherwise Qwen may reject
