 ...
 * an OpenAI Chat Completions request body (what Qwen's DashScope upstream accepts
 * at https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions).
 *
 * Codex sends:
 *   { model, instructions, input: string | InputItem[], tools?, stream, store, ... }
 *
 * DashScope accepts the Chat Completions shape:
...
export function responsesToChatCompletions(
  ...
  // Qwen thinking-mode "all-or-nothing" rule: if ANY assistant message in the
...
function convertInputItem(item: unknown, out: ChatMessage[], state: ConvertState): void {
  ...
  if (type === 'reasoning') {
    ...
    // (Qwen thinking mode requires reasoning_content to be passed back).
  ...
  if (type === 'function_call') {
    ...
    // tool messages can follow as a block (Qwen rejects an assistant tool_calls
  ...
  if (typeof content === 'string') {
    ...
    for (const part of content) {
      ...
      // input_image / image_url / file / etc. silently skipped — the Qwen
