...
describe('DeepSeek catalog pricing schedule', () => {
  ...
  it('shows the previous price before the schedule takes effect', () => {
    expect(resolveModelPrice(model('deepseek-v4-pro'), Date.parse('2026-08-22T15:59:59.999Z'))).toMatchObject({
      inputPrice: 0.435,
      outputPrice: 0.87,
  ...
  it('shows current off-peak and peak prices using UTC windows', () => {
    expect(resolveModelPrice(model('deepseek-v4-pro'), Date.parse('2026-08-22T16:00:00Z'))).toMatchObject({
      inputPrice: 0.66,
      outputPrice: 1.98,
      cacheReadPrice: 0.022,
      period: 'off-peak'
    })
    expect(resolveModelPrice(model('deepseek-v4-pro'), Date.parse('2026-08-24T06:00:00Z'))).toMatchObject({
      inputPrice: 1.32,
      outputPrice: 3.96,
  ...
  it('shows Beijing weekends as off-peak even during UTC peak hours', () => {
    expect(resolveModelPrice(model('deepseek-flash'), Date.parse('2026-09-12T02:00:00Z'))).toMatchObject({
      inputPrice: 0.15,
      outputPrice: 0.6,
  ...
  it('shows Flash prices while keeping Pro prices after the canceled retirement', () => {
    expect(resolveModelPrice(model('deepseek-flash'), Date.parse('2026-09-10T14:00:00+08:00'))).toMatchObject({
      inputPrice: 0.3,
      outputPrice: 1.2,
    ...
    expect(resolveModelPrice(model('deepseek-v4-pro'), canceledRetirement - 1)).toMatchObject({ inputPrice: 1.32, outputPrice: 3.96, period: 'peak' })
    expect(resolveModelPrice(model('deepseek-v4-pro'), canceledRetirement)).toMatchObject({
      inputPrice: 0.66,
      outputPrice: 1.98,
      cacheReadPrice: 0.022,
      period: 'off-peak'
    })
    expect(resolveModelPrice(model('deepseek-v4-pro'), canceledRetirement + 2 * 60 * 60_000)).toMatchObject({
      inputPrice: 1.32,
      outputPrice: 3.96,
      cacheReadPrice: 0.044,
      period: 'peak'
...
describe('current provider model catalog', () => {
  ...
  it('switches Gemini frontier Flash cards to standard pricing in 2027', () => {
    expect(resolveModelPrice(model('gemini-3.8-flash'), Date.parse('2027-01-01T00:00:00Z'))).toMatchObject({
      inputPrice: 1.5,
      outputPrice: 7.5,
      cacheReadPrice: 0.15
    })
    expect(resolveModelPrice(model('gemini-3.6-flash'), Date.parse('2026-12-31T23:59:59.999Z'))).toMatchObject({
      inputPrice: 0.75,
      outputPrice: 3.75,
      cacheReadPrice: 0.075
    })
