...
核心区别一句话：**sub2api 是更完整的商业化拼车 SaaS；model-bridge 已具备中转、管理后台、邀请用户、钱包和余额扣费的基础闭环，但仍保持轻量自托管定位。**
...
| 3 | **用户钱包 + 余额扣费闭环** | ✅ | ✅ 基础能力已实现 | 已支持管理员调账、用户余额、usage 扣费 |
...
1. **支付接入**（支付宝 / 微信 / Stripe 至少一个）—— 补齐自动充值 / 收款闭环。
2. **充值订单与回调审计**（订单表、支付回调幂等、余额入账流水）。
