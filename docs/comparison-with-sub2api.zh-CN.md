...
| 项目 | 定位 |
|---|---|
| **model-bridge**（本项目）| 自托管的多服务商 AI API 中转平台，偏「个人 / 小团队自用」的轻量网关 |
| **sub2api** | AI API 网关 + **拼车 SaaS 平台**，主打「订阅配额分发 + 内置计费收款」 |

核心区别一句话：**sub2api 是带「商业化收费 / 多租户」的 SaaS 网关；model-bridge 是纯粹的「中转 + 管理后台」工具。**
...
|---|---|:---:|:---:|---|
| 1 | **内置支付 / 收款系统**（EasyPay / 支付宝 / 微信 / Stripe）| ✅ | ❌ | 待开发 · 拼车 SaaS 的核心 |
| 2 | **多租户 / 用户自助注册** | ✅ | ❌ 仅单一 admin | 待开发 · 所有商业化功能的前提 |
| 3 | **用户钱包 + 余额扣费闭环** | ✅ | ⚠️ 仅 per-key 美元配额上限 | 待开发 |
...
多账户管理、API Key 分发、Token 级用量统计与成本计算、粘性会话调度、并发控制、限流、Web 管理仪表盘、OAuth 授权、PostgreSQL、Claude / OpenAI(Codex) / Gemini 支持。
...
1. **多租户用户体系**（用户表 + 用户登录）—— 商业化的前提。
2. **用户钱包 + 余额扣费**（在现有 `usage_logs` + `quotaUsed` 上扩展）。
3. **支付接入**（支付宝 / 微信 / Stripe 至少一个）。
