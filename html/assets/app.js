...
(() => {
  ...
  const brand = (publicPage = false) =>
    `<a class="brand" href="${publicPage ? "landing.html" : "index.html"}" aria-label="Model Bridge ${publicPage ? "首页" : "总览"}"><img src="assets/model-bridge-logo.png" alt=""><div>${state.settings.name === "Model Bridge" ? "Model<span>Bridge</span>" : esc(state.settings.name)}</div></a>`;
  function navigation() {
